...
function matchSideClass(isPlayed: boolean, isWinner: boolean): string {
  if (isPlayed && isWinner) return "text-gray-900";
  if (isPlayed) return "text-gray-400";
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white px-4 py-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {isPlayed ? (
          <span
            className={`min-w-14 shrink-0 text-center font-bold tabular-nums ${scoreSize} text-gray-900`}
          >
            {match.scoreA} – {match.scoreB}
          </span>
        ) : (
          <span className="min-w-14 shrink-0 text-center text-sm font-medium text-gray-300">
            vs
          </span>
        )}
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      {groupPoules.map((poule) => {
        ...
        return (
          <div key={poule.id} className="overflow-hidden rounded-2xl border border-gray-100">
            <div className="border-b border-gray-100 bg-gray-50 px-4 py-3">
              <h3 className="font-bold text-gray-900">{poule.name}</h3>
              {poule.description && (
                <p className="mt-0.5 text-xs text-gray-400">{poule.description}</p>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {pouleTeams.map((team, i) => (
                    ...
                    >
                      <td className="py-3 pl-4 pr-2 text-center tabular-nums text-gray-400">{i + 1}</td>
                      <td className="px-2 py-3 font-medium text-gray-900">{team.name}</td>
                      <td className="px-2 py-3 text-center tabular-nums text-gray-500">{team.played}</td>
                      <td className="px-2 py-3 text-center tabular-nums text-gray-500">{team.won}</td>
                      <td className="hidden px-2 py-3 text-center tabular-nums text-gray-500 sm:table-cell">{team.drawn}</td>
                      <td className="px-2 py-3 text-center tabular-nums text-gray-500">{team.lost}</td>
                      <td className="hidden px-2 py-3 text-center tabular-nums text-gray-500 sm:table-cell">{team.goalsFor}</td>
                      <td className="hidden px-2 py-3 text-center tabular-nums text-gray-500 sm:table-cell">{team.goalsAgainst}</td>
                      ...
                      <td className="px-2 py-3 pr-4 text-center tabular-nums font-bold text-gray-900">
                        {team.points}
                      </td>
                    </tr>
                  ))}
...
function TabFinale({
  ...
  // Earlier rounds (R16, R8, QUARTER) grouped
    ...
    <div className="flex flex-col gap-10">
      ...
      {winnerName && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-5 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">🏆 {winnerName}</p>
        </div>
      )}
    ...
    <div className="rounded-2xl border border-gray-100 bg-gray-50 px-5 py-4">
      ...
      <div className="flex flex-col gap-0.5">
        {[team.speler1, team.speler2, team.speler3, team.speler4].map((speler) => (
          <p key={speler} className="text-xs text-gray-400">
            {speler}
          </p>
        ))}
      </div>
...
function TabReglement({ rules }: Readonly<{ rules: { description: string } | null }>) {
  ...
  return (
    <div className="max-w-2xl rounded-2xl border border-gray-100 bg-gray-50 px-6 py-5">
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-white px-6">
        <div className="text-center">
          <h1 className="mb-3 text-2xl font-bold text-gray-900">Toernooi niet gevonden</h1>
          <Link href="/toernooi" className="text-sm font-semibold text-amber-500 hover:underline">
    ...
    <div className="bg-white">
      ...
      <div className="border-b border-gray-200">
        <div className="mx-auto max-w-6xl px-6 py-10">
          <Link
            href="/toernooi"
            className="mb-4 inline-flex items-center gap-1 text-xs font-semibold text-gray-400 hover:text-gray-600"
          >
            ← Alle toernooien
          </Link>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-amber-500">
                {tournament.year}
                {tournament.isActive && (
                  <span className="ml-2 rounded-full bg-amber-500 px-2 py-0.5 text-white">
                    Actief
                  </span>
                )}
              </p>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-gray-500">
              <span>
                <strong className="tabular-nums text-gray-900">{teams.length}</strong> teams
              </span>
              <span>
                <strong className="tabular-nums text-gray-900">{poules.length}</strong> poules
              </span>
              <span>
                <strong className="tabular-nums text-gray-900">{playedCount}</strong>/
