...
export default function RegisterPage() {
  ...
  // Pre-validate the code from the URL on mount
  useEffect(() => {
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              type="text"
              value={inviteCode}
              onChange={(e) => { setInviteCode(e.target.value); setCodeRole(null); setCodeError(""); }}
              onBlur={handleCodeBlur}
              placeholder="bijv. A3F9D21B7C"
              required
              disabled={loading}
              className={`w-full rounded-xl border px-3.5 py-2.5 text-sm text-ink placeholder:text-ink-2 focus:outline-none focus:ring-2 disabled:bg-surface disabled:cursor-not-allowed transition-colors font-mono tracking-widest uppercase ${
                codeError
                  ? "border-red-400 focus:border-red-400 focus:ring-red-200"
                  : codeRole
                  ? "border-green-400 focus:border-green-400 focus:ring-green-200"
                  : "border-rule bg-paper focus:border-pink focus:ring-pink/15"
              }`}
            />
